...
| `0x6e` | Generic Linux enrollment commit operation; two instances also appeared after the fourth accepted update in one successful Windows A21 runtime trace. | Linux CFG and Windows runtime | High for opcode/order; low for equivalence of the two Windows operations | Inferred: CommitEnrollment in the generic Linux path; exact Windows runtime roles unresolved | Differently sized Windows replies preceded Hello success |
