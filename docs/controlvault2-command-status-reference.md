...
| `0x6e` | Generic Linux enrollment commit operation; successful Windows A21 tracing observed two nested CSS/raw CommitEnrollment calls after four successful updates, matching the earlier two-opcode USB sequence. | Linux CFG, Windows USB runtime, and Windows function trace | High for operation family and order; low for equivalence of the two calls | CommitEnrollment | Both returned zero, but earlier USB replies differed in size and call arguments remain unobserved |
